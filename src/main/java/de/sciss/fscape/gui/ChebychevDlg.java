...
/**
 *  Processing module for multiband waveshaping using
...
topLevel: try {
		...
		// ---- calculate filters ----
			...
			midFreq				= Param.transform( pr.para[ PR_MIDFREQ ], Param.ABS_HZ, pr.para[ PR_LOFREQ ], null ).val;
			hiFreq				= Math.min( inStream.rate * 0.2475,
								  Param.transform( pr.para[ PR_HIFREQ ], Param.ABS_HZ, pr.para[ PR_LOFREQ ], null ).val );
			freqFactor			= Math.pow( 2.0, 1.0 / pr.para[ PR_BANDSPEROCT ].val );
