...
// }
		...
		// ---- misc inits ----
			d1				= Param.transform( pr.para[ PR_MINCHUNK ], Param.ABS_MS, null, null ).val;
...
//			minChunk		= Math.max( 2, (int) (d1 + 0.5) );
			...
			d1				= Param.transform( pr.para[ PR_TOLERANCE ], Param.ABS_AMP, new Param( 1.0, Param.ABS_AMP ), null ).val;
			if( d1 > 1.0 ) {
				topTol		= (float) d1;
				botTol		= (float) (1.0 / d1);
			} else {
...
// System.out.println(" :::: "+j+" ; "+k+"; "+m +"; "+ (Math.min( k >> 1, j - off )) );
				chunkLenParam.val	= AudioFileDescr.samplesToMillis( inStream, m );
				d1					= Param.transform( pr.para[ PR_CROSSFADE ], Param.ABS_MS, chunkLenParam, null ).val;
