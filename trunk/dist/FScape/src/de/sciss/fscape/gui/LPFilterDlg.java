...
/**
 *  Processing module supposed to do linear
 *	predictive coding. Well, it does something,
 *	but I don't know what, at least something
 *	different from linear predictive coding.
 *
 *  @author		Hanns Holger Rutz
 *  @version	0.64, 06-Dec-04
 */
public class LPFilterDlg
...
// -------- public Methoden --------
	...
	public LPFilterDlg()
	{
		super( "Linear Predictive Filtering" );
		init2();
	}
	...
	gui.addLabel( new GroupLabel( "LP Settings", GroupLabel.ORIENT_HORIZONTAL,
		...
		con.weightx		= 0.1;
		con.gridwidth	= 1;
		gui.addLabel( new JLabel( "Analysis Size", JLabel.RIGHT ));
		con.weightx		= 0.4;
		...
		con.weightx		= 0.1;
		con.gridwidth	= 1;
		gui.addLabel( new JLabel( "Synthesis Size", JLabel.RIGHT ));
		con.weightx		= 0.4;
		...
		con.weightx		= 0.1;
		gui.addLabel( new JLabel( "Window", JLabel.RIGHT ));
		con.gridwidth	= GridBagConstraints.REMAINDER;
		con.weightx		= 0.4;
		...
		con.weightx		= 0.1;
		con.gridwidth	= 1;
		gui.addLabel( new JLabel( "Window Step", JLabel.RIGHT ));
		con.weightx		= 0.4;
...
// -------- Processor Interface --------
	...
	protected void process()
	{
		...
		float[]				lpCoeffs, win;
		int					stepSize, winSize, bufSize, lpOrder, readOff, overBufSize;
		boolean				skip;
...
topLevel: try {
		...
		// .... check running ....
			...
			lpOrder			= (int) pr.para[ PR_LPORDER ].val;
			d1				= Param.transform( pr.para[ PR_STEPSIZE ], Param.ABS_MS, null, null ).val;
			winSize			= Math.max( lpOrder, (int) (AudioFileDescr.millisToSamples( inStream, d1 ) + 0.5) );
			stepSize		= Math.max( 1, winSize >> pr.intg[ PR_OVERLAP ]);
			winSize			= stepSize << pr.intg[ PR_OVERLAP ];	// evtl. correction
			d1				= Param.transform( pr.para[ PR_CALCLEN ], Param.ABS_MS, null, null ).val;
			bufSize			= Math.max( winSize + lpOrder, (int) (AudioFileDescr.millisToSamples( inStream, d1 ) + 0.5) );
			readOff			= bufSize - stepSize;
			inBuf			= new float[ inChanNum ][ bufSize ];
			outBuf			= new float[ inChanNum ][ winSize ];
			overBufSize		= winSize - stepSize;
			overBuf			= new float[ inChanNum ][ overBufSize ];
			lpCoeffs		= new float[ lpOrder ];
			win				= Filter.createFullWindow( winSize, pr.intg[ PR_WINDOW ]);
			Util.clear( inBuf );
			Util.clear( overBuf );

// System.out.println( "lpOrder "+lpOrder+"; winSize "+winSize+"; stepSize "+stepSze+"; bufSize "+bufSize+"; readOff "+readOff );
		...
		// =================== CORE ===================
			...
			while( threadRunning && (framesWritten < inLength) ) {
				...
				for( ch = 0; ch < inChanNum; ch++ ) {	// buffer shift
					System.arraycopy( inBuf[ ch ], stepSize, inBuf[ ch ], 0, readOff );
				}
				len	= Math.min( stepSize, inLength - framesRead );
			...
			// .... check running ....
				...
				for( ch = 0; ch < inChanNum; ch++ ) {
					convBuf1 = inBuf[ ch ];
					convBuf2 = outBuf[ ch ];
					lpCoeffs( convBuf1, 0, bufSize, lpCoeffs, lpOrder );
//					fixRoots( lpCoeffs, lpOrder );	// XXX
//					lpFilter( convBuf1, stepSize, lpCoeffs, lpOrder, convBuf2, 0, winSize );
					linearPrediction( convBuf1, bufSize - winSize, lpCoeffs, lpOrder, convBuf2, 0, winSize );
					if( pr.bool[ PR_RESIDUAL ]) {
						for( i = 0, j = bufSize - winSize; i < winSize; ) {
							convBuf2[ i++ ] -= convBuf1[ j++ ];
						}
					}
					convBuf1 = overBuf[ ch ];
					Util.mult( win, 0, convBuf2, 0, winSize );
					Util.add( convBuf1, 0, convBuf2, 0, stepSize );
					for( i = stepSize, j = 0; i < overBufSize; i++, j++ ) {
						convBuf1[ j ] = convBuf1[ i ] + convBuf2[ i ];
					}
					System.arraycopy( convBuf2, i, convBuf1, j, stepSize );
				}
				progOff		+= len;
			// .... progress ....
				setProgression( (float) progOff / (float) progLen );
			// .... check running ....
				...
				if( skip ) {
					...
					len	= Math.min( stepSize, inLength - framesWritten );
					if( floatF != null ) {
						for( ch = 0; ch < inChanNum; ch++ ) {
							convBuf1 = outBuf[ ch ];
