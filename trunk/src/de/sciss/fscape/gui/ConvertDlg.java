...
//		outputChanged();
	...
	/*
	 *	Settings-GUI in Abhängigkeit von der jeweiligen Konvertierungs-Art
	 */
	protected PropertyGUI createGUI( int type )
	{
		...
		String	window		= "lbWindow;ch,pr"+PRN_WINDOW+",itHamming,itBlackman,itKaiser ß=4,itKaiser ß=5,itKaiser ß=6,itKaiser ß=8\n";
...
//			setFont( getFont() );
	...
	/**
	 *	automatischer Output Name bei geändertem Input oder Output-Type
...
// -------- Processor Interface --------
	...
	protected void process()
	{
		...
		// FFT, DFT, FWT
		double				freqScale		= 0f;	// DFT: Freq.factor von einem Band zum nächsthöheren
		float				img, real;
		float[]				smpData			= null;
		float[]				smpData2		= null;	// gewindowte Version
		float[][]			cos				= null;	// 1. Dimension = Freq-Offset, 2. Dimension = Zeit-Offset
		float[][]			sin				= null;
		float[]				fCos, fSin;			// cos/sin mit aufgelöster erster Dimension
...
//						break;
					case PRS_TRANSFORM_DFT:						// .... discrete fourier w/ log scale ....
						...
						// des untersten Bandes errechnen und ggf. nach unten so anpassen, daß die
