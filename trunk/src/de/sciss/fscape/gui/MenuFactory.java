...
//				if( type.equals( "FScP" )) { // == Constants.OSTypePrefs ) {
					...
					preset.load();
					className	= preset.getProperty( DocumentFrame.PROP_CLASS );
					if( className != null ) {
...
//						args[ 3 ] = new IOException( "Wrong file format. Missing entry '" + DocumentFrame.PROP_CLASS + "'" );
						pt.setException( new IOException( "Wrong file format. Missing entry '" + DocumentFrame.PROP_CLASS + "'" ));
						return FAILED;
					}
...
//				final Session			doc		= (Session) args[ 2 ];
				...
				final DocumentFrame		procWin	= doc.getFrame();
