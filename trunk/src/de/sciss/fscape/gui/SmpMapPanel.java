		...
		synchronized( smpMap ) {
			...
			for( int index = smpMap.size() - 1; index >= 0; index-- ) {
				...
				if( Math.abs( smp.velLo.val - smpMap.hSpace.min ) < Constants.suckyDoubleError ) {		// expand
					...
					smp.velLo.val = hSpace.fitValue( smp.velLo.val );
				}
				if( Math.abs( smp.velHi.val - smpMap.hSpace.max ) < Constants.suckyDoubleError ) {		// expand
					smp.velHi.val = hSpace.max;
				} else {
					smp.velHi.val = hSpace.fitValue( smp.velHi.val );
				}
				if( Math.abs( smp.freqHi.val - smpMap.vSpace.max ) < Constants.suckyDoubleError ) {		// expand
					smp.freqHi.val = vSpace.max;
				} else {
					smp.freqHi.val = vSpace.fitValue( smp.freqHi.val );
				}
				if( Math.abs( smp.freqLo.val - smpMap.vSpace.min ) < Constants.suckyDoubleError ) {		// expand
					smp.freqLo.val = vSpace.min;
				} else {
					smp.freqLo.val = vSpace.fitValue( smp.freqLo.val );
				}

				if( ((smp.velHi.val  - smp.velLo.val  + Constants.suckyDoubleError) > hSpace.inc) &&	// not if too small
					((smp.freqHi.val - smp.freqLo.val + Constants.suckyDoubleError) > vSpace.inc) ) {
...
// -------- Mouse Listener Methoden --------
	...
	public void mouseReleased( MouseEvent e )
	{
		...
		if( dragState ) {
			...
			smp.freqHi.val	= dragTopLeft.y;
			smp.freqLo.val	= dragBottomRight.y;
			smp.velHi.val	= dragBottomRight.x;
			smp.velLo.val	= dragTopLeft.x;
...
// -------- MouseMotion Listener Methoden --------
	...
	public void mouseDragged( MouseEvent e )
	{
		...
		if( !dragState ) {		// check if distance is ok to start drag
			...
			dragTopLeft		= new DoublePoint( dragSmp.velLo.val, dragSmp.freqHi.val );
			dragBottomRight	= new DoublePoint( dragSmp.velHi.val, dragSmp.freqLo.val );
		...
		if( (dragType & DRAG_LEFT) != 0 ) {
			if( e.getX() != dragOriginX ) {
				...
				dragTopLeft.x = dragSmp.velLo.val;	// keine Rundungsfehler wenn derselbe Screen-Pixel
			}
		}
		if( (dragType & DRAG_TOP) != 0 ) {
			if( e.getY() != dragOriginY ) {
				...
				dragTopLeft.y = dragSmp.freqHi.val;
			}
		}
		if( (dragType & DRAG_RIGHT) != 0 ) {
			if( e.getX() != dragOriginX ) {
				...
				dragBottomRight.x = dragSmp.velHi.val;
			}
		}
		if( (dragType & DRAG_BOTTOM) != 0 ) {
			if( e.getY() != dragOriginY ) {
				...
				dragBottomRight.y = dragSmp.freqLo.val;
			}
		}
...
// -------- private Methoden --------
	...
	protected void recalcScreenBox( SmpBox sb )
	{
		...
		synchronized( smpMap ) {
			...
			smp		= smpMap.getSample( index );
			if( (sb != null) && (smp != null) ) {
				loc1 = paramSpaceToScreen( smp.velLo.val, smp.freqHi.val );
				loc2 = paramSpaceToScreen( smp.velHi.val, smp.freqLo.val );
				loc3 = paramSpaceToScreen( smp.velLo.val, smp.base.val );
...
//				  (smpMap.vSpace.max - dy) / (smpMap.vSpace.max - smpMap.vSpace.min) + 0.5);
	...
	protected boolean findFreePlaceAround( int x, int y, DoublePoint tl, DoublePoint br )
	{
		int			num, i, j;
		SmpZone		smp;
		double		velMid, freqMid;				// Space-Repräsentation der Parameter x, y
		...
		synchronized( smpMap ) {
			...
			for( i = 0; i < num; i++ ) {	// step through all zones
				...
				iHoriz	= (smp.velLo.val < velHi) && (smp.velHi.val > velLo);
				iVert	= (smp.freqHi.val > freqLo) && (smp.freqLo.val < freqHi);
				if( !iHoriz || !iVert ) continue;		// no intersection
				
				cutNum	= 0;
				if( (smp.velHi.val <= velMid) && (smp.velHi.val < velHi) ) {		// cut left border
					velCutLo[ cutNum ]	= smp.velHi.val;
					velCutHi[ cutNum ]	= velHi;
					freqCutLo[ cutNum ]	= freqLo;
					freqCutHi[ cutNum ]	= freqHi;
					cutNum++;
				}
				if( (smp.velLo.val >= velMid) && (smp.velLo.val > velLo) ) {		// cut right border
					velCutLo[ cutNum ]	= velLo;
					velCutHi[ cutNum ]	= smp.velLo.val;
					freqCutLo[ cutNum ]	= freqLo;
					freqCutHi[ cutNum ]	= freqHi;
					cutNum++;
				}
				if( (smp.freqLo.val >= freqMid) && (smp.freqLo.val > freqLo) ) {	// cut top border
					velCutLo[ cutNum ]	= velLo;
					velCutHi[ cutNum ]	= velHi;
					freqCutLo[ cutNum ]	= freqLo;
					freqCutHi[ cutNum ]	= smp.freqLo.val;
					cutNum++;
				}
				if( (smp.freqHi.val <= freqMid) && (smp.freqHi.val < freqHi) ) {	// cut bottom
					velCutLo[ cutNum ]	= velLo;
					velCutHi[ cutNum ]	= velHi;
					freqCutLo[ cutNum ]	= smp.freqHi.val;
					freqCutHi[ cutNum ]	= freqHi;
					cutNum++;
				}
...
// g2.dispose();
									...
									DoublePoint otl, DoublePoint obr, int dragType )
	{
		...
		freqSpan	= smp.freqHi.val - smp.freqLo.val;
		velSpan		= smp.velHi.val  - smp.velLo.val;
		...
		synchronized( smpMap ) {
			...
			do {
				for( i = 0, intersected = false; i < num; i++ ) {	// step through all zones
					...
					iHoriz	= (smp2.velLo.val < br.x) && (smp2.velHi.val > tl.x);
					iVert	= (smp2.freqHi.val > br.y) && (smp2.freqLo.val < tl.y);
					if( !iHoriz || !iVert ) continue;		// no intersection
	
					intersected = true;
	
					orientL	= obr.x <= smp2.velLo.val + Constants.suckyDoubleError;
					orientR	= otl.x >= smp2.velHi.val - Constants.suckyDoubleError;
					orientT	= obr.y >= smp2.freqHi.val - Constants.suckyDoubleError;
					orientB	= otl.y <= smp2.freqLo.val + Constants.suckyDoubleError;
					...
					if( orientT && ((dragType & DRAG_BOTTOM) != 0) ) {
						br.y = smp2.freqHi.val;
						if( (dragType & DRAG_TOP) != 0 ) {
							tl.y = br.y + freqSpan;
						}
					}
					if( orientB && ((dragType & DRAG_TOP) != 0) ) {
						tl.y = smp2.freqLo.val;
						if( (dragType & DRAG_BOTTOM) != 0 ) {
							br.y = tl.y - freqSpan;
						}
					}
