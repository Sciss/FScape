...
// -------- private Variablen --------
	...
	protected String				name;		// wird zurückgegeben über toString()
			...
			InterruptedException
	{
		...
		synchronized( this ) {
			try {
				while( fr == null ) {
					try {
						if( stream.framesReadable() != 0 ) {	// auch -1, löst ja EOFException aus
							fr = stream.readFrame();
							if( linked.state == STATE_WAITING ) {
								// geschachtelte synchronized nur, wenn klar ist, daß der andere wartet!
								synchronized( linked ) {
			...
			InterruptedException
	{
		...
		synchronized( this ) {
			try {
				while( fr != null ) {
					try {
						if( stream.framesWriteable() != 0 ) {	// auch -1, löst ja EOFException aus
